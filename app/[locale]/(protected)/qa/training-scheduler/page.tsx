                            ...
                            <div className="flex items-center bg-muted rounded-lg p-1 gap-0.5">
                                {VIEW_OPTIONS.map(({ id, label, icon: Icon }) => (
                                    <button key={id} onClick={() => setView(id)}
                                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs transition-all cursor-pointer border-none ${
                                            view === id ? 'bg-card text-foreground shadow-sm font-medium' : 'text-muted-foreground hover:text-foreground bg-transparent'
                                        }`}>
                                ...
                                {STATUSES.map(s => {
                                    ...
                                    return (
                                        <button key={s} onClick={() => setFilterStatus(s)}
                                            className={`px-3 py-1 rounded-full text-xs border transition-all cursor-pointer ${filterStatus === s
                                                ? (s === 'All' ? 'bg-foreground text-background border-foreground font-medium' : `${cfg.bg} ${cfg.text} ${cfg.border} font-medium`)
