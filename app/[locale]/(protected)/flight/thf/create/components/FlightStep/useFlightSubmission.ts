...
export const useFlightSubmission = (flightData: any, onSave: (data: any) => void, goNext: () => void) => {
  ...
  const onSubmit = (data: Step1FormInputs) => {
    const pushThfData = {
      ...
      arrivalAtaDate: data.ata?.trim() ? combineFormToUtcDatetime(data.arrivalDate, data.ata) : "",
      departureFlightNo: data.flightDeparture || "",
      departureStdDate: data.departureDate && data.std ? combineFormToUtcDatetime(data.departureDate, data.std) : "",
      departureAtdDate: data.departureDate && data.atd?.trim() ? combineFormToUtcDatetime(data.departureDate, data.atd) : "",
      bayNo: data.bay || "",
