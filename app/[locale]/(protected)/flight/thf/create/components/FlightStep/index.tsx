  ...
  const onSubmit = async (data: Step1FormInputs) => {
    try {
      ...
      const payload: PushThfRequest = {
        ...
        arrivalAtaDate: data.ata?.trim() ? combineToUtcDatetime(convertDateToBackend(data.arrivalDate), data.ata) : "",
        departureFlightNo: data.flightDeparture,
        departureStdDate: data.departureDate && data.std ? combineToUtcDatetime(convertDateToBackend(data.departureDate), data.std) : "",
        departureAtdDate: data.departureDate && data.atd?.trim() ? combineToUtcDatetime(convertDateToBackend(data.departureDate), data.atd) : "",
        bayNo: data.bay || '',
