...
const COMPLETENESS: Record<CompletenessStatus, { label: string; className: string }> = {
  draft: { label: "Draft", className: "bg-slate-100 text-slate-600 border-slate-200" },
  incomplete: { label: "Incomplete", className: "bg-amber-100 text-amber-800 border-amber-200" },
  complete: { label: "Complete", className: "bg-emerald-100 text-emerald-800 border-emerald-200" },
};

function Chip({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-block rounded-full bg-white/70 px-2 py-0.5 text-[11px] text-blue-900 ring-1 ring-inset ring-blue-200">
      {children}
    </span>
  );
}
...
export function AircraftEngineRefPanel({ context, refId, className }: AircraftEngineRefPanelProps) {
  ...
  const title = context === "authorization" ? "Authorization group" : "Aircraft system config";
  ...
  return (
    <div className={cn("overflow-hidden rounded-xl border border-blue-200 bg-blue-50/70", className)}>
      ...
      <div className="flex items-center gap-2 border-b border-blue-200/70 px-3.5 py-2">
        <span className="text-[11px] font-semibold uppercase tracking-wider text-blue-700">{title}</span>
          ...
          (group ? (
            <>
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-sm font-bold text-blue-950">{group.groupLabel}</span>
                <span className="text-[10px] text-blue-400">{group.groupId}</span>
                <span
                  className={cn(
                    "ml-auto inline-block rounded-full border px-2 py-0.5 text-[10px] font-medium",
                    COMPLETENESS[group.completenessStatus].className,
                  )}
                >
                  {COMPLETENESS[group.completenessStatus].label}
                </span>
              </div>

              <div>
                <div className="mb-1 text-[10px] uppercase tracking-wider text-blue-500">Engines (rolled up)</div>
                <div className="flex flex-wrap gap-1">
                  {group.engineListCached.length ? (
                    group.engineListCached.map((e) => <Chip key={e}>{e}</Chip>)
                  ) : (
              ...
              <div>
                <div className="mb-1 text-[10px] uppercase tracking-wider text-blue-500">Member series</div>
                <div className="flex flex-wrap gap-1">
                  {group.memberCombinationIds.length ? (
                    combinations
                      .filter((c) => group.memberCombinationIds.includes(c.id))
                      .map((c) => <Chip key={c.id}>{c.series ? `${c.familyCode}-${c.series}` : c.familyCode}</Chip>)
                  ) : (
                    <span className="text-xs text-blue-300">—</span>
                  )}
                </div>
            ...
            <p className="text-xs text-blue-400">ไม่พบ Authorization group “{refId}”</p>
          ))}
