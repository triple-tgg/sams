...
const sections = [
    ...H.coverPage({
        docNo: "SAMS-QA-SRS-10",
        docTitle: "Constraints & Assumptions",
        subtitleText: "ข้อจำกัดและสมมติฐานของระบบ — โมดูล QA",
    }),

    H.h1("1. ข้อจำกัด (Constraints)"),
    H.h2("1.1 Technical Constraints"),
    H.makeTable(
    ...
    H.h2("1.3 Regulatory Constraints"),
    H.makeTable(
        ["ID", "ข้อจำกัด", "รายละเอียด"],
        [
    ...
    H.h2("1.4 Performance Constraints"),
    H.makeTable(
    ...
    H.note("รายละเอียดเพิ่มเติมใน SRS-05 (Non-Functional Requirements)"),

    H.pageBreak(),

    H.h1("2. สมมติฐาน (Assumptions)"),
    H.h2("2.1 User & Operational Assumptions"),
    H.makeTable(
        ["ID", "สมมติฐาน"],
        [
    ...
    H.h2("2.2 Technical Assumptions"),
    H.makeTable(
        ["ID", "สมมติฐาน"],
        [
    ...
    H.h1("3. Dependencies (สิ่งที่ระบบพึ่งพา)"),
    H.diagramPlaceholder("Internal & External Dependencies", "graph TB — QA → Auth/Master/FileStorage (internal) + SMTP/HR/Backup (external)"),
    H.makeTable(
    ...
    H.h1("4. Out of Scope"),
    H.h2("4.1 ไม่อยู่ในระบบ QA Module"),
    H.makeTable(
        ["หมวด", "อยู่ที่ไหน"],
        [
    ...
    H.h2("4.2 ฟีเจอร์ที่ไม่อยู่ใน Phase 1"),
    H.makeTable(
    ...
    H.h1("5. Risk Mitigation Linked to Constraints"),
    H.diagramPlaceholder("Constraint → Mitigation Map", "graph LR — TC-01/BC-02/RC-02/UA-05 → Mitigation strategies"),
    H.note("รายละเอียด Risk เพิ่มเติมใน SRS-13 (Risk & Impact)"),

    H.spacer(),
    H.p("— จบเอกสาร SAMS-QA-SRS-10 —", { italics: true, color: "999999" }),
];

const doc = H.buildDoc({ docNo: "SAMS-QA-SRS-10", sections });
H.saveDoc(doc, __dirname + "/../10-Constraints-Assumptions/SAMS-QA-SRS-10-Constraints-Assumptions.docx");
